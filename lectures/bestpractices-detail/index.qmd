...
#| echo: true
#| eval: true
snakefmt --compact-diff linting2.smk
```
...
`snakefmt` modifies files inplace so run with `--diff` or
`--compact-diff` to view changes.
...
- show example; point out that linter will complain
...
Here point to the example repo; point out that it does testing
...
#| eval: true
#| echo: false
tree -a -F -L 2 -I '.snakemake|LICENSE|.git|resources*references.fasta|resources*README.md|Dockerfile|environment.yaml|.gitignore|.gitattributes|.editorconfig|.pre-commit-config.yaml|config*config.yaml|config*samples.tsv|config*reads.tsv|.ipynb_checkpoints|.myprofile|logs|reports|results|interim|*.~undo-tree~|*.png|*.zip|*.html|.github' | sed -e "s/\.\.\/\.\.\///g" | grep -v directories
```
...
## What does it do? {style="font-size: 0.75em; "}
...
## Dry-run the test suite  {style="font-size: 0.8em; "}

```{bash }
#| label: snakemake-byoc-2021-dry-run-echo
#| eval: false
#| echo: false
snakemake -s workflow/Snakefile --directory .test -n -F --quiet rules
```
...
## Snakefile {style="font-size: 0.7em; "}
...
## rmarkdown scripts {style="font-size: .8em;"}
...
```{python code=readLines("workflow/rules/inputfunctions.smk")[1:39], filename="inputfunctions.smk"}
...
```{python code=readLines("workflow/rules/mapping.smk")[34:73], filename="mapping.smk"}
...
## Configuration schemas to [https://json-schema.org/](https://json-schema.org)  {style="font-size: .9em;"}
...
## Configuration schemas {style="font-size: .75em;"}
...
## Workflow rst template files {style="font-size: 0.8em;"}
...
#| echo: false
#| eval: true
cp .test/report.html ../bestpractices-detail/
```
...
```{=html}
<div id="wrap">
    <iframe id="smkreport" style="width: 1000px; height: 600px;" src="report.html" title="Snakemake report"></iframe>
</div>
```
...
## Pre-commit - for the git power user {style="font-size: 0.8em;"}
...
and see how many warnings/errors you get!
