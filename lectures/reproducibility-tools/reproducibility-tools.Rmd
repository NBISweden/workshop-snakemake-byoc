...
* Each Conda package is also available as Docker and Singularity images (_e.g._ check http://biocontainers.pro for Conda packages from the .green[bioconda] channel)

--

* Many other Docker images are also available on .green[DockerHub] (https://hub.docker.com/)
...
* Snakemake can .green[automatically] generate a Docker container image that contains all
  Conda environments used by the rules of the workflow
...
* The workflow is run in .green[Snakemake] until a rule to run the foreign workflow
  is reached

--

* In this rule, Snakemake .green[hands over] to the other workflow management system 
  via the directive `handover`
