...
* Track code changes with **Git** and share on [GitHub](https://github.com)/[GitLab](https://gitlab.com/)/[Codeberg](https://codeberg.org/) (not this talk)
* Make workflow reproducible with a workflow manager (**Snakemake**, **Nextflow**, **WDL**)
...
* Most relevant for us: The [Bioconda](https://bioconda.github.io/) channel
...
* This gives us reproducibility (if you include version numbers).
...
* Write an environment file (`environment.yaml`) that includes **all tools used by the workflow** ([see also](https://github.com/NBISweden/snakemake_best_practice)):
  ...
  channels:
    ...
    - bioconda
  dependencies:
    - snakemake=9.21.0   # ← Snakemake is part of the environment
  ...
    - multiqc=1.35   # ← Version numbers for reproducibility
    - samtools=1.23
  ```
...
* Create the environment file, such as `envs/bwa.yaml`
* Add the `conda:` directive to the rule:
  ...
  rule create_bwa_index:
      ...
      shell:
          "bwa index {input}"
  ```
* Run `snakemake` with `--use-conda` (or `--software-deployment-method conda`)
