...
format:
  nbis-course-revealjs:
    footer: Snakemake BYOC - Running Snakemake
  nbis-course-logo-revealjs:
    footer: Snakemake BYOC - Running Snakemake
---
...
- Examples with Snakefiles and code to run in
  [examples](https://download-directory.github.io?url=https://github.com/NBISweden/workshop-snakemake-byoc/tree/main/lectures/running_snakemake/examples)
  subdirectory (click to download)
...
## Example 2 - ex2.smk  {style="font-size: .8em;"}
...
## Example 3 - ex3.smk {style="font-size: .7em;"}
...
## Example 3 - ex3.smk: other handy options {style="font-size: .7em;"}
...
`--rulegraph` is a convenient way of getting an overview of the
workflow
...
#| output: asis
#| echo: false
snakemake -s ex3.smk --filegraph | dot -T svg | grep -v "<!DOCTYPE" | \
    grep -v "dtd" | \
...
## Distribution and reproducibility - ex4.smk {style="font-size: .8em;"}
...
## Distribution and reproducibility - ex5.smk {style="font-size: .8em;"}
...
## Distribution and reproducibility - ex6.smk {style="font-size: .8em;"}
...
## About resources {style="font-size: .8em;"}
...
The resources `mem_mb` and `disk_mb` now have been assigned default
values.
...
## Adding threads and resources to workflow - ex7.smk {style="font-size: .7em;"}
...
## Setting rule-specific resources {style="font-size: .8em;"}
...
snakemake -F -n -s ex7.smk --default-resources mem_mb=2000 \
...
```{yaml code=readLines("local/config.yaml") }
...
## Snakemake profiles (Snakemake version <8) {style="font-size: .7em;"}
...
## Snakemake Slurm profile {style="font-size: .6em;"}
...
```{python code=readLines("myprofile/settings.json"), filename="myprofile/settings.json" }
...
```{yaml code=readLines("myprofile/config.yaml"), filename="myprofile/config.yaml" }
#| label: slurm-config
#| eval: false
#| echo: true
```
...
```{python code=readLines("myprofile/CookieCutter.py")[4:39], filename="myprofile/CookieCutter.py"}
...
## New features - time formatting and sbatch parameters {style="font-size: .8em;"}
...
With the latest version of the Slurm profile, you can do the
following:
...
## Snakemake version 8 updates

#### Workflow changes

- `dynamic` deprecated $\rightarrow$ `checkpoint` directive
- `version` deprecated $\rightarrow$ `conda`, `container` integration
- `subworkflow` removed $\rightarrow$ `module` directive

##### Remote providers

<!-- markdownlint-disable MD013 -->

Remote providers have been removed in favor of [storage
plugins](https://snakemake.readthedocs.io/en/stable/snakefiles/storage.html#storage-support)
...
`--use-conda`, `--use-singularity` deprecated $\rightarrow$ use
`--software-deployment-method conda`, `--software-deployment-method
apptainer`, or `--software-deployment-method conda apptainer` for both

##### Execution backends

<!-- markdownlint-disable MD013 -->

Execution backends have been moved to [execution
plugins](https://snakemake.github.io/snakemake-plugin-catalog/index.html).
For instance, the `slurm` plugin would be called as `--executor
slurm`.

<!-- markdownlint-enable MD013 -->

## Snakemake and Slurm {style="font-size: .8em;"}

#### Setup

```{bash }
#| label: snakemake-v8-plugin-install
#| echo: true
#| eval: false
...
```{yaml }
#| label: snakemake-executor-plugin-config
#| echo: true
#| eval: false
default-resources:
...
```{bash }
#| label: snakemake-executor-slurm-command
#| echo: true
#| eval: false
