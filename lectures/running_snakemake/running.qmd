...
Let's start with a snakefile that contains a single rule for doing an
alignment with bwa:
...
snakemake -f -n -p -s ex6.smk --default-resources
```
...
another rule, the keyword `threads` and set resources for one of the
rules:
...
snakemake -F -n -s ex7.smk --default-resources --set-resources bwa_mem_wildcard:runtime=1000 \
...
snakemake -s ex7.smk -F --ri -k \
		  ...
		  --default-resources --set-resources bwa_mem_wildcard:runtime=1000 \
		  bwa_mem_wildcard:mem_mb=6000 samtools_merge_bam:runtime=100 \
		  --set-threads bwa_mem_wildcard=4 -c 8
```
...
This is getting illegible and it is tedious to write. What to do? Put
it in a bash script?
...
instance, `Ctrl-b d` will detach (exit) from the session. See the
[tmux
...
```{bash code=readLines("myprofile/settings.json") }
...
```{bash code=readLines("myprofile/config.yaml") }
#| label: slurm-config
#| eval: false
```
